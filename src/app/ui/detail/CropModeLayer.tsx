...
import { transformRect, oppositeCorner, cutLineWithPolygon, getCornerVectorOfRect, vectorFromPoint, getRectFromVectors,
    roundVector, horizontalAdjacentCorner, verticalAdjacentCorner, isVectorInPolygon, VectorLike } from 'app/util/GeometryUtil'
...
export default class CropModeLayer extends React.Component<Props> {
    ...
    private onCornerDrag(corner: Corner, point: Point, isFinished: boolean) {
        ...
        const projectedPoint = vec2.transformMat4(vec2.create(), vectorFromPoint(point), invertedCameraMatrix)
        const oppositePoint = getCornerVectorOfRect(prevCropRect, oppositeCorner[corner])
        ...
        const xCutPoint = limitPointToPolygon(getCornerVectorOfRect(prevCropRect, verticalAdjacentCorner[corner]), oppositePoint, texturePolygon, true)
        if (Math.abs(oppositePoint[0] - xCutPoint[0]) < Math.abs(oppositePoint[0] - nextCornerPoint[0])) {
            nextCornerPoint[0] = xCutPoint[0]
        }
        const yCutPoint = limitPointToPolygon(getCornerVectorOfRect(prevCropRect, horizontalAdjacentCorner[corner]), oppositePoint, texturePolygon, true)
        if (Math.abs(oppositePoint[1] - yCutPoint[1]) < Math.abs(oppositePoint[1] - nextCornerPoint[1])) {
            nextCornerPoint[1] = yCutPoint[1]
        }

        const cropRect = getRectFromVectors(roundVector(nextCornerPoint), oppositePoint)
...
function limitPointToPolygon(point: vec2, referencePoint: VectorLike, texturePolygon: VectorLike[], goBeyoundPoint = false): vec2 {
